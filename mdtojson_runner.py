...
class OCRAnalyzer:
    ...
    def get_pcg_accounts(self, account_class: int = 6) -> str:
        ...
        logger.info(f"Récupération des comptes PCG Standard classe {account_class}")
        ...
        if not response.data:
            logger.warning(f"Aucun compte trouvé pour la classe {account_class}")
        ...
        logger.info(f"Chargé {len(response.data)} comptes de classe {account_class}")
    ...
    def inject_variables(self, prompt: str, company: Optional[Dict[str, Any]]) -> str:
        ...
        logger.info(f"Liste des comptes injectée: {len(accounts_list.splitlines())} comptes")
        
        return prompt
    
    def call_claude(self, prompt: str, markdown_content: str) -> Tuple[str, Dict[str, Any]]:
        """Appelle l'API Claude avec retry et backoff exponentiel, extended thinking activé."""
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.info(
                    f"Réponse Claude reçue en {duration:.1f}s - "
